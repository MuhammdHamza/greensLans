...
describe('Tax', () => {
  beforeEach(() => {
    cy.then(() => {
      ...
      cy.get("[type='text']").type("lee2@greenstem.com.my");
  ...
  it('Forex', () => {
    ...
    cy.get("[class='dx-item dx-treeview-item'] span:contains('AR Debit Note')").click({ force: true });
  ...
  it('Not Tax Inclusive', () => {
    ...
    cy.get("[class='dx-item dx-treeview-item'] span:contains('Account Receivable')").click();
    cy.get("[class='dx-item dx-treeview-item'] span:contains('AR Debit Note')").click({ force: true });    searchWithCustomerCode({ code: "Z0015" })
    cy.get('body').then($body => {
      if ($body.find("[class='errorMessage-popup-body']").length > 0) {
        cy.get("[class='errorMessage-popup-body']").should('be.visible').then(() => {
          // cy.get("[class='dx-button-text'] span:contains('Close')").click();
